import {
  W
} from "./chunk-XHWGKFQG.js";
import "./chunk-NMHVPKGY.js";
import {
  axios_default,
  isAxiosError,
  isCancel,
  mergeConfig
} from "./chunk-UWVM3XHT.js";
import {
  require_react
...
var client = {
  get: (url, data = {}, config = {}) => request(mergeConfig2("get", url, data, config)),
  post: (url, data = {}, config = {}) => request(mergeConfig2("post", url, data, config)),
  patch: (url, data = {}, config = {}) => request(mergeConfig2("patch", url, data, config)),
  put: (url, data = {}, config = {}) => request(mergeConfig2("put", url, data, config)),
  delete: (url, data = {}, config = {}) => request(mergeConfig2("delete", url, data, config)),
  use(axios) {
    axiosClient = axios;
    return client;
  },
...
var mergeConfig2 = (method2, url, data, config) => ({
  url,
  method: method2,
  ...config,
  ...["get", "delete"].includes(method2) ? {
    params: merge_default({}, data, config == null ? void 0 : config.params)
  } : {
    data: merge_default({}, data, config == null ? void 0 : config.data)
  }
});
var request = (userConfig = {}) => {
  ...
  ].reduce((config2, callback) => callback(config2), userConfig);
  ...
  return axiosClient.request(config).then(async (response) => {
    ...
    const statusHandler = resolveStatusHandler(config, status) ?? ((response2) => response2);
    ...
    const statusHandler = resolveStatusHandler(config, error.response.status) ?? ((_, error2) => Promise.reject(error2));
...
var abortMatchingRequests = (config) => {
  ...
  (_a = abortControllers[config.fingerprint]) == null ? void 0 : _a.abort();
  delete abortControllers[config.fingerprint];
  return config;
};
var refreshAbortController = (config) => {
  if (typeof config.fingerprint !== "string" || config.signal || config.cancelToken || !config.precognitive) {
    return config;
  }
...
var resolveContentType = (config) => {
  var _a, _b, _c;
  return ((_a = config.headers) == null ? void 0 : _a["Content-Type"]) ?? ((_b = config.headers) == null ? void 0 : _b["Content-type"]) ?? ((_c = config.headers) == null ? void 0 : _c["content-type"]) ?? (hasFiles(config.data) ? "multipart/form-data" : "application/json");
};
...
var resolveUrl = (url) => typeof url === "string" ? url : url();
var resolveMethod = (method2) => typeof method2 === "string" ? method2.toLowerCase() : method2();
...
var createValidator = (callback, initialData = {}) => {
  ...
  const resolveConfig2 = (globalConfig, instanceConfig, data = {}) => {
    ...
    const validate2 = Array.from(config.validate ?? touched);
    return {
      ...instanceConfig,
      ...
      onValidationError: (response, axiosError) => {
        [
          ...setValidated([...validated, ...validate2]),
          ...setErrors(merge_default(omit_default({ ...errors }, validate2), response.data.errors))
        ].forEach((listener) => listener());
        return config.onValidationError ? config.onValidationError(response, axiosError) : Promise.reject(axiosError);
      },
      onSuccess: (response) => {
        setValidated([...validated, ...validate2]).forEach((listener) => listener());
        return config.onSuccess ? config.onSuccess(response) : response;
      },
      onPrecognitionSuccess: (response) => {
        [
          ...setValidated([...validated, ...validate2]),
          ...setErrors(omit_default({ ...errors }, validate2))
        ].forEach((listener) => listener());
      ...
      onBefore: () => {
        const beforeValidationHandler = config.onBeforeValidation ?? ((newRequest, oldRequest) => {
          return newRequest.touched.length > 0 && !isEqual_default(newRequest, oldRequest);
        });
...
var forgetFiles = (data) => {
  ...
  Object.keys(newData).forEach((name) => {
    ...
    if (Array.isArray(value)) {
      newData[name] = value.filter((value2) => !isFile(value2));
      return;
    }
    if (typeof value === "object") {
...
// node_modules/laravel-precognition-react/dist/index.js
var import_react = __toESM(require_react());
var useForm = (method2, url, input, config = {}) => {
  const originalData = (0, import_react.useRef)(null);
  if (originalData.current === null) {
    originalData.current = cloneDeep_default(input);
  }
  const originalInputs = (0, import_react.useRef)(null);
  if (originalInputs.current === null) {
    originalInputs.current = Object.keys(originalData);
  }
  const payload = (0, import_react.useRef)(originalData.current);
  ...
  const [data, setData2] = (0, import_react.useState)(() => cloneDeep_default(originalData.current));
  const validator = (0, import_react.useRef)(null);
  if (validator.current === null) {
    validator.current = createValidator((client2) => client2[resolveMethod(method2)](resolveUrl(url), payload.current, config), input).on("validatingChanged", () => {
  ...
  const resolveSubmitConfig = (config2) => ({
    ...config2,
    precognitive: false,
    onStart: () => {
      setProcessing(true);
    ...
    onValidationError: (response, error) => {
      ...
      return config2.onValidationError ? config2.onValidationError(response) : Promise.reject(error);
    }
  });
  const form = {
    data,
    setData(key, value) {
      if (typeof key === "object") {
        ...
        const newData = cloneDeep_default(payload.current);
        payload.current = set_default(newData, key, value);
        setData2(payload.current);
      }
      return form;
    },
    touched(name) {
    ...
    validate(name, config2) {
      if (typeof name === "object" && !("target" in name)) {
        config2 = name;
        name = void 0;
      }
      if (typeof name === "undefined") {
        validator.current.validate(config2);
      } else {
        name = resolveName(name);
        validator.current.validate(name, get_default(payload.current, name), config2);
      }
      return form;
    },
    validating,
    ...
    hasErrors,
    setErrors(errors2) {
      validator.current.setErrors(errors2);
      return form;
    },
    forgetError(name) {
    ...
    reset(...names) {
      const original = cloneDeep_default(originalData.current);
      if (names.length === 0) {
        payload.current = original;
        setData2(original);
      } else {
        names.forEach((name) => set_default(payload.current, name, get_default(original, name)));
        setData2(payload.current);
      }
    ...
    processing,
    async submit(config2 = {}) {
      return client[resolveMethod(method2)](resolveUrl(url), payload.current, resolveSubmitConfig(config2));
    },
    validateFiles() {
...
// node_modules/laravel-precognition-react-inertia/dist/index.js
var import_react3 = __toESM(require_react());
var useForm2 = (method2, url, inputs, config = {}) => {
  const booted = (0, import_react3.useRef)(false);
  const inertiaForm = W(inputs);
  const precognitiveForm = useForm(method2, url, inputs, config);
  ...
  const transformer = (0, import_react3.useRef)((data) => data);
  if (!booted.current) {
  ...
  const form = Object.assign(inertiaForm, {
    ...
    validate(name, config2) {
      ...
      if (typeof name === "undefined") {
        precognitiveForm.validate(config2);
      } else {
        precognitiveForm.validate(name, config2);
      }
      return form;
    },
    setValidationTimeout(duration) {
    ...
    submit(submitMethod = {}, submitUrl, submitOptions) {
      ...
      submitOptions = isPatchedCall ? submitMethod : submitOptions;
      submitUrl = isPatchedCall ? resolveUrl(url) : submitUrl;
      submitMethod = isPatchedCall ? resolveMethod(method2) : submitMethod;
...
lodash-es/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=laravel-precognition-react-inertia.js.map
